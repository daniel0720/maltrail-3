...
NAME = "Maltrail"
VERSION = "0.13.13"
SERVER_HEADER = "{}/{}".format(NAME, VERSION)
DATE_FORMAT = "%Y-%m-%d"
ROTATING_CHARS = ('\\','|', '|', '/', '-')
TIMEOUT = 30
FRESH_IPCAT_DELTA_DAYS = 10
USERS_DIR = os.path.join(os.path.expanduser("~"), ".{}".format(NAME.lower()))
...
IPCAT_URL = "https://raw.githubusercontent.com/client9/ipcat/master/datacenters.csv"    # 文件里面是IP地址段和对应的数据中心
...
SESSION_COOKIE_NAME = "{}_sessid".format(NAME.lower())
...
SYSTEM_LOG_DIR = "/var/log" if not subprocess._mswindows else "C:\\Windows\\Logs"
...
IGNORE_EVENTS = set()
STATIC_IPCAT_LOOKUPS = {"shadowserver.org": ("184.105.139.66-184.105.139.126", "184.105.247.194-184.105.247.254", "74.82.47.1-74.82.47.63", "216.218.206.66-216.218.206.126"), 
                        "labs.rapid7.com": ("71.6.216.32-71.6.216.63",), 
                        "shodan.io": ("66.240.192.138", "66.240.236.119", "71.6.135.131", "71.6.165.200", "71.6.167.142", "82.221.105.6", "82.221.105.7", "85.25.43.94", "85.25.103.50", "93.120.27.62", "104.131.0.69", "104.236.198.48", "162.159.244.38", "188.138.9.50", "198.20.69.74", "198.20.69.98", "198.20.70.114", "198.20.87.98", "198.20.99.130", "208.180.20.97", "209.126.110.38"), 
                        "eecs.umich.edu": ("141.212.121.0-141.212.121.255", "141.212.122.0-141.212.122.255"), 
                        "netsec.colostate.edu": ("129.82.138.12", "129.82.138.31", "129.82.138.32", "129.82.138.33", "129.82.138.34", "129.82.138.44"), 
                        "ant.isi.edu": ("128.9.168.98", "203.178.148.18", "203.178.148.19"), 
                        "eecs.berkeley.edu": ("169.229.3.89", "169.229.3.90", "169.229.3.91", "169.229.3.92", "169.229.3.93", "169.229.3.94"), 
                        "openresolverproject.org": ("204.42.253.2", "204.42.254.5"), 
                        "opensnmpproject.org": ("204.42.253.130",), 
                        "openntpproject.org": ("204.42.253.131",), 
                        "openssdpproject.org": ("204.42.253.132",), 
                        "projectblindferret.com": ("107.150.52.82-107.150.52.86",), 
                        "kudelskisecurity.com": ("185.35.62.0-185.35.62.255",), 
                        "riskiq.com": ("64.125.239.0-64.125.239.255",), 
                        "comsys.rwth-aachen.de": ("137.226.113.0-137.226.113.63",), 
                        "sba-research.org": ("98.189.26.18",)}
...
# get physical memory of windows/linux system
...
# check if there is enough free memory, need 384MB
def check_memory():
    print("[?] at least {}MB of free memory required".format(CHECK_MEMORY_SIZE / 1024 / 1024))
    try:
...
def read_config(config_file):
    ...
    if not os.path.isfile(config_file):
        exit("[!] missing configuration file {}".format(config_file))
    else:
        print("[i] using configuration file {}".format(config_file))
    ...
    try:
        ...
        for line in content.split("\n"):
            line = line.strip('\r')
            line = re.sub(r"\s*#.*", "", line)  # 去掉注释部分
            if not line.strip():                # 如果本行为注释，则处理完后line为空，这时跳转到下一行
                continue

            if line.count(' ') == 0:            # 只有USERS, IP_ALIASES, USER_WHITELIST这三个配置语句中没有空格
                if re.search(r"[^\w]", line):   # 匹配到配置内容，如USERS的内容和IP_ALIASES的内容
                    if array == "USERS":
                        exit("[!] invalid USERS entry {}\n[?] (hint: add whitespace at start of line)".format(line))
                    else:
                        exit("[!] invalid configuration (line: {})".format(line))
                array = line.upper()
                config[array] = []
                continue

            if array and line.startswith(' '):  # 将IP_ALIASES和USERS的配置内容存放到列表中
                config[array].append(line.strip())
                continue
            else:                               # 处理USERS, IP_ALIASES, USER_WHITELIST这三个配置以外的配置，解析出配置名name和配置的值value
                array = None
                try:
            ...
            _ = os.environ.get("{}_{}".format(NAME.upper(), name))
            if _:
                value = _
        ...
        pass
    # 以上将配置文件的内容读取到config结构中，接下来处理配置文件

    for option in ("MONITOR_INTERFACE", "CAPTURE_BUFFER", "LOG_DIR"):
        if not option in config:
            exit("[!] missing mandatory option {} in configuration file {}".format(option, config_file))
    
    for entry in (config.USERS or []):
        if len(entry.split(":")) != 4:
            exit("[!] invalid USERS entry {}".format(entry))
        if re.search(r"\$\d+\$", entry):
            exit("[!] invalid USERS entry {}\n[?] (hint: please  update PBKDF2 hashes to SHA256 in your configuration file)".format(entry))
    ...
    if config.PROCESS_COUNT and subprocess._mswindows:
...
# 从worst_asns.txt中读取网络前缀、掩码和名称，存放在WORST_ASNS字典中，字典的键是IP地址的前8字节，值是网络前缀、掩码和名称组成的元组
def read_worst_asn():
...
# 从bogon_ranges.txt中读取网络前缀、掩码，存放在BOGON_RANGES字典中，字典的键是IP地址的前8字节，值是网络前缀、掩码组成的元组
...
if __name__ == '__main__':
    read_config('/home/daniel/workspace/maltrail-3/maltrail.conf')